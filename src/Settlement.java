...
public class Settlement {
    ...
    public ArrayList<Road> findRoads(String name) {
        ...
        for (Road current : getAllRoads()) {  // checks every road for the road stored in name
            if (current.getName().equalsIgnoreCase(name))
                roadsFound.add(current);
        }
    ...
    public void delete(Road road) throws IllegalArgumentException {
        Settlement source = road.getSourceSettlement();
        Settlement destination = road.getDestinationSettlement();
        source.roads.remove(source.roads.indexOf(road));
        destination.roads.remove(destination.roads.indexOf(road));
